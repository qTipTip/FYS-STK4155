...
clf_ol = GridSearchCV(LogisticRegression(), ol_params, cv=5, verbose=True, n_jobs=-1, scoring='f1')
clf_l1 = GridSearchCV(LogisticRegression(), l1_params, cv=5, verbose=True, n_jobs=-1, scoring='f1')
clf_l2 = GridSearchCV(LogisticRegression(), l2_params, cv=5, verbose=True, n_jobs=-1, scoring='f1')
...
plt.ylabel('F1 score')
plt.scatter(clf_l1.best_params_['C'], clf_l1.best_score_, label=rf'$\lambda_1 = {clf_l1.best_params_["C"]:.3f}$')
plt.scatter(clf_l2.best_params_['C'], clf_l2.best_score_, label=rf'$\lambda_2 = {clf_l2.best_params_["C"]:.3f}$')
plt.legend()
...
plt.savefig('credit_card_classification_f1.pdf')
plt.show()
